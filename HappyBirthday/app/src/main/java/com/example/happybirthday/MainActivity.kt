...
fun BirthdayGreetingWithText(message: String, form: String) {
    Column {
        Text(
            text = message,
            fontSize = 30.sp,
            modifier = Modifier.fillMaxWidth().wrapContentWidth(Alignment.CenterHorizontally).padding(start = 32.dp, top=16.dp)
        )
        Text(
            text = form,
            fontSize = 24.sp,
...
fun BirthdayGreetingWithImage(message: String, from: String) {
    ...
    Box {
        ...
        BirthdayGreetingWithText(message = message, form = from)
    }
